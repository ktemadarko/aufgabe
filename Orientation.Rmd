...
ui<-fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      fluidRow(
        column(textOutput(outputId = "resultDisplay2020"),width=3),
        column(textOutput(outputId = "resultDisplay2021"),width=3),
        column(textOutput(outputId = "resultDisplay2022"),width=3),
        column(textOutput(outputId = "resultDisplay2023"),width=3))
    )
    )
...
#create a function that takes in a year and returns the count of rows with that year

queryResult<-function(year){
  outputName<-paste0("resultDisplay",year)

  output[[outputName]] <- renderText({
    as.character(dbGetQuery(mydb, paste0('SELECT COUNT(*) FROM roche_products WHERE "Filing date" = ',year)))
